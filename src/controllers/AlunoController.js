...
export class AlunoController {
    ...
    async listarPorId(req, res) {
        try {
            ...
            return aluno ? res.status(200).json(aluno) : res.status(400).json("Aluno não encontrado");

        } catch (error) {
            return res.status(500).json({ message: "Erro ao buscar aluno", erro: error.message });
        }
    }
    ...
    async alterarAluno(req, res) {
        ...
        try {
            ...
            return res.status(404).json({ message: "Aluno não encontrado ou erro ao atualizar", error: error.message });
        }
    };
